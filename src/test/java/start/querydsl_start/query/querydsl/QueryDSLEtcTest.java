...
public class QueryDSLEtcTest {
    ...
    void initData() {
        ...
        Firm f2 = new Firm("노무라증권");
        Firm f3 = new Firm("삼성증권");
        em.persist(f1);
        em.persist(f2);
        em.persist(f3);
    ...
    @Test
    @DisplayName("QueryDSL-[Case-일반] 테스트")
    @Transactional
    @Rollback(false)
    void testQuertDSL_CaseNormal()
    {
        //WHERE
        QTrader t = QTrader.trader;
        List<String> results = jpaQueryFactory
                .select(t.firm.name
                        .when("삼성증권").then("국내증권사")
                        .when("노무라증권").then("일본증권사")
                        .otherwise("기타증권사")
                )
                .from(t)
                .fetch();
    ...
    @Test
    @DisplayName("QueryDSL-[Case-고급] 테스트")
    @Transactional
    @Rollback(false)
    void testQuertDSL_CaseAdvanced()
    {
        //WHERE
        QTrader t = QTrader.trader;

        List<String> results = jpaQueryFactory
                .select(t.username.concat("_").concat(
                                new CaseBuilder()
                                .when(t.salary.between(10001L,20000L)).then("고액연봉자")
                                .when(t.salary.between(8001L,10000L)).then("중간연봉자")
                                .when(t.salary.between(5000L,8000L)).then("저연봉자")
                                .otherwise("촉탁")
                ))
                .from(t)
                .fetch();
