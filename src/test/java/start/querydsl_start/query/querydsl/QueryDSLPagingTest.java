...
public class QueryDSLPagingTest {
    ...
    @Transactional
    @Rollback(false)
    void testQuertDSL_PAGING()
    {
        //WHERE
        QTrader t = QTrader.trader;

        QueryResults<Trader> results = jpaQueryFactory
                .select(t)
                .from(t)
                .orderBy(t.age.asc(), t.salary.asc().nullsLast())
                .offset(3)
                .limit(5)
                .fetchResults();
        ...
        results.getResults().stream().forEach( trader -> log.info("{}",trader));
